local M = {}
local picker_state = require('kago.picker.state')
local ui = require('kago.picker.ui')
local preview = require('kago.picker.preview')
local select = require('kago.picker.select')
local win = require('kago.picker.window')

vim.api.nvim_set_hl(0, 'PickerMatch', { link = 'Special' })

---@class kago.picker.Config
---@field debounce_ms integer
---@field height_ratio number
---@field width_ratio number
---@field exclude_globs string[]

---@type kago.picker.Config
M.config = {
  debounce_ms = 150,
  height_ratio = 0.8,
  width_ratio = 0.9,
...
local sources = {
  ...
  grep = require('kago.picker.sources.grep'),
  buf_lines = require('kago.picker.sources.buf_lines'),
  tree = require('kago.picker.sources.tree'),
  git = require('kago.picker.sources.git'),
  select = require('kago.picker.sources.select'),
}
...
local function setup_autocmds()
  state.augroup = vim.api.nvim_create_augroup('kago_picker', { clear = true })
