...
describe('explorer filter', function()
  ...
  it('scans paths with smart case, literals, hidden files and symlink cycles', function()
  ...
  it('filters interactively and restores tree state when cleared', function()
    ...
    eq('kago-explorer-filter', vim.bo[input].filetype)
    ...
    eq(1, #lines(buf))
    feed('/<CR>')
    equal(vim.bo.filetype, 'kago-explorer-filter')
    feed('<Esc>')
    feed('<Esc>')
    feed('/Target<Esc>')
    assert(vim.wait(5000, function()
    ...
    feed('<CR>')
    eq(root .. '/closed/deep/Target.lua', vim.api.nvim_buf_get_name(0))
    explorer.close()
    explorer.open({ root = root })
  ...
  it('renders process spawn errors in the sidebar', function()
  ...
  it('discards stale scans after reload and disposal', function()
    local system = vim.system
    local pending = {}
