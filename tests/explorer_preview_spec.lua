...
local function preview_win()
  for _, win in ipairs(vim.api.nvim_list_wins()) do
    if vim.bo[vim.api.nvim_win_get_buf(win)].filetype == 'kago-explorer-preview' then
      return win
    end
  end
end
...
describe('explorer preview', function()
  ...
  it('previews unsaved buffers and cleans related UI through sidebar lifecycle', function()
    vim.o.columns = 100
    vim.o.hidden = true
    ...
    equal(
      vim.bo[buf].syntax == 'lua' or rawget(vim.treesitter.highlighter.active, buf) ~= nil,
      true
    )
  ...
  it('bounds file previews and discards late updates', function()
    vim.o.columns = 100
    local p = preview.new()
    p.enabled = true
